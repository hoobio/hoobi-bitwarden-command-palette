...
The extension uses a custom clipboard service that provides two security features for sensitive data.
...
After copying sensitive data, the clipboard is automatically cleared after a configurable delay. The timer is smart — it checks whether the clipboard contents have been replaced by another application before clearing. If the user has already copied something else, the clear is skipped.
